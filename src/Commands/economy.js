    ...
    if (args[0]) {
        ...
        if (isItem) {
            if (!obj[message.author.id]) {
                ...
                fx.econ.econActionEmbed("Error", "You don't own that item.", message, "#FF0000");
                fs.writeFile("src/Data/economy/users.json", JSON.stringify(obj), () => {} );
            }
            else if (amount > obj[message.author.id].inv[item]) {
                fx.econ.econActionEmbed("Error", "Not enough items!", message, "#FF0000");
            }
            else {
                obj[message.author.id].inv[item] -= amount;
                fx.econ.econActionEmbed("Tossed!", `You threw away ${amount} ${item}(s).`, message, "#333333");
